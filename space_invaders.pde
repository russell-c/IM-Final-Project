...
class Rect {  //constructing a class for the player and obstacle objects
  ...
  boolean collideRect(Rect other) {
    if (this.x+this.w >= other.x && this.x+this.w <= other.x+other.w) { //if inside the obstacle bound coming from the left side
...
void draw() {
  ...
  if (worldRecord < 10) { 
    ...
    player.move(width-closestX,closestY); //move the player 
  ...
  for (int i = 0; i<obstacles.size(); i++) { //loop through list of obstacles
    ...
    if (temp.y>height) { //if obstacle if off screen remove it from obstacle list
      obstacles.remove(i);
    }
